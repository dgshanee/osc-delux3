...
int main()
{
  ...
  Rectangle settingsRect = (Rectangle){ screenWidth - screenWidth/3.0f, 0, screenWidth/3.0f, screenHeight };
  ...
  while(!WindowShouldClose()){
    ...
    // snprintf(timeStr, sizeof(timeStr), "%f", time);

    // DrawText(timeStr, 20, 20, 20, BLUE );
    ...
    BeginScissorMode(curves[0].start, curves[1].end, curves[0].end-curves[0].start, curves[1].start - curves[1].end + 32 );
      ...
      DrawLine(ballPos.x, ballPos.y, ballPos.x + cosf((ballRotation + 90) * DEG2RAD) * ballSize.x, ballPos.y + sinf((ballRotation + 90)*DEG2RAD)*ballSize.y, RED);
    ...
    //Bounds
    DrawRectangleLines(curves[0].start, curves[1].end, curves[0].end - curves[0].start, curves[1].start - curves[1].end + 32, GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR)));

    if(showHelp)
    {
      showHelp = !GuiWindowBox((Rectangle) {margin, margin, settingsRect.x - 2*margin, curves[1].end-2*margin}, "help");

      Rectangle helpTextRect = (Rectangle) { 2 * margin, 2 * margin+RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT, settingsRect.x - 4 - 4*margin, 0};
      // DrawRectangleLines(helpTextRect.x, helpTextRect.y, helpTextRect.width, helpTextRect.height, RED);
      ...
      GuiLabel((Rectangle){helpTextRect.x, helpTextRect.y + helpTextRect.height, helpTextRect.width, fontSize}, "- Left clicl to move/add point");
    ...
    //help button
    if (GuiButton((Rectangle){contentRect.x, contentRect.y + contentRect.height + scrollOffset.y, contentRect.width, 1.5 * fontSize }, "Curve controls help")) showHelp = !showHelp;    
